            ...
            unsafe {
                ...
                match id {
                    ...
                    SYSCALL_EXIT => {
                        ...
                        println!("[Kernel] User App exited with code: {}", exit_code);
                        // 重啟 Shell
...
pub extern "C" fn rust_main() -> ! {
    ...
    unsafe {
        ...
        // [新增] 儲存到全域變數，給 elf.rs 用
        mm::page_table::KERNEL_PAGE_TABLE = root_ptr;

        println!("[Kernel] Root Page Table at: {:p}", root);

        // --- 建立映射 (Identity Mapping) ---
        
        // A. 映射 UART (0x1000_0000)
        // Kernel 讀寫 UART 不需要 PTE_U，因為是 System Call (M-Mode) 在操作
        println!("[Kernel] Mapping UART...");
        mm::page_table::map(root, 0x1000_0000, 0x1000_0000, PTE_R | PTE_W);

        // B. 映射 CLINT (0x0200_0000)
        println!("[Kernel] Mapping CLINT...");
        let mut addr = 0x0200_0000;
        while addr < 0x0200_FFFF {
            mm::page_table::map(root, addr, addr, PTE_R | PTE_W);
            addr += 4096;
        }

        // C. 映射 RAM (Kernel Code + Heap + Shell Task)
        // [關鍵修正] 加上 PTE_U !!!
        // 因為 task1 (Shell) 是 User Mode，且它位在 Kernel 的記憶體區段中
        // 如果沒有 PTE_U，Shell 一執行就會 Page Fault (mcause=12)
        println!("[Kernel] Mapping RAM (RWX + User)...");
        let start = 0x8000_0000;
        let end   = 0x8040_0000; // 映射 4MB
        let mut addr = start;
        while addr < end {
        ...
        let satp_val = (8 << 60) | root_ppn; // Mode=Sv39
        
        println!("[Kernel] Enabling MMU (satp = 0x{:x})...", satp_val);
        ...
        println!("[Kernel] MMU Enabled Successfully!");
        ...
        println!("[OS] Jumping to User Mode (Virtual Memory ON)...");
